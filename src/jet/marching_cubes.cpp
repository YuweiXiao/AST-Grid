...
#include "jet/marching_cubes_table.h"
#include "jet/marching_squares_table.h"
...
template <typename T>
inline T distanceToZeroLevelSet(T phi0, T phi1) {
...
inline Vector3f grad(const CT& grid,
    ssize_t i, ssize_t j, ssize_t k, const Vector3f& invGridSize) {
    Vector3f ret;
    ssize_t ip = i + 1;
    ssize_t im = i - 1;
    ssize_t jp = j + 1;
    ...
    ret.x() = 0.5f * invGridSize.x() * (grid->get(ip, j, k) - grid->get(im, j, k));
    ret.y() = 0.5f * invGridSize.y() * (grid->get(i, jp, k) - grid->get(i, jm, k));
    ret.z() = 0.5f * invGridSize.z() * (grid->get(i, j, kp) - grid->get(i, j, km));
    return ret;
}

inline Vector3f safeNormalize(const Vector3f& n) {
...
// i        i+1         2i   2i+1  2i+2
//
inline size_t globalEdgeId( size_t i, size_t j, size_t k,
...
// i        i+1         2i   2i+1  2i+2
//
inline size_t globalVertexId(size_t i, size_t j, size_t k,
...
static void singleSquare(
    ...
    REAL phi0, phi1, alpha;
    Vector3f pos, pos0, pos1;
    Vector3f e[4];
    ...
    for (itrTri = 0; itrTri < 4; ++itrTri) {
        ...
        for (int j = 0; j < 3; ++j) {
            ...
            if (queryVertexId(*vertexMap, vKey, &vId)) {
                ...
                mesh->addUv(Vector2f(0, 0));  // empty texture coord...
...
static void singleCube(
    const std::array<REAL, 8>& data,
    const std::array<size_t, 12>& edgeIds,
    const std::array<Vector3f, 8>& normals,
    const BoundingBox3& bound,
    MarchingCubeVertexMap* vertexMap,
    TriangleMesh3* mesh,
    REAL isoValue) {
    int itrVertex, itrEdge, itrTri;
    ...
    Vector3f pos, pos0, pos1, normal, normal0, normal1;
    REAL phi0, phi1;
    REAL alpha;
    Vector3f e[12], n[12];
...
void marchingCubes( const CellCenteredScalarGrid3Ptr& grid, REAL gridSize,
    const Vector3f& origin, TriangleMesh3* mesh, REAL isoValue, int bndFlag) {

    MarchingCubeVertexMap vertexMap;

    const Size3 dim = grid->resolution();
    const Vector3f invGridSize = Vector3f(1.0 / gridSize, 1.0 / gridSize, 1.0 / gridSize);

    auto pos = [origin, gridSize](ssize_t i, ssize_t j, ssize_t k)->Vector3f{
        return origin + Vector3f(i * gridSize, j * gridSize, k * gridSize);
    };
    ...
    for (ssize_t k = 0; k < dimz - 1; ++k) {
        for (ssize_t j = 0; j < dimy - 1; ++j) {
            for (ssize_t i = 0; i < dimx - 1; ++i) {
                std::array<REAL, 8> data;
                std::array<size_t, 12> edgeIds;
                std::array<Vector3f, 8>  normals;
                BoundingBox3 bound;

                data[0] = grid->get(i, j, k);
                data[1] = grid->get(i + 1, j, k);
                data[4] = grid->get(i, j + 1, k);
                data[5] = grid->get(i + 1, j + 1, k);
                data[3] = grid->get(i, j, k + 1);
                data[2] = grid->get(i + 1, j, k + 1);
                data[7] = grid->get(i, j + 1, k + 1);
                data[6] = grid->get(i + 1, j + 1, k + 1);
    ...
    if (bndFlag
        ...
        for (ssize_t j = 0; j < dimy-1; ++j) {
            for (ssize_t i = 0; i < dimx-1; ++i) {
                ...
                data[0] = grid->get(i + 1, j, k);
                data[1] = grid->get(i, j, k);
                data[2] = grid->get(i, j + 1, k);
                data[3] = grid->get(i + 1, j + 1, k);

                if (bndFlag & kDirectionZNegative) {
                ...
                k = dimz - 2;
                data[0] = grid->get(i, j, k + 1);
                data[1] = grid->get(i + 1, j, k + 1);
                data[2] = grid->get(i + 1, j + 1, k + 1);
                data[3] = grid->get(i, j + 1, k + 1);

                if (bndFlag & kDirectionZPositive) {
                    normal = Vector3f(0, 0, 1);
    ...
    vertexMap.clear();
    if (bndFlag
        & (kDirectionXNegative | kDirectionXPositive)) {
        for (ssize_t k = 0; k < dimz-1; ++k) {
            for (ssize_t j = 0; j < dimy-1; ++j) {
                ssize_t i = 0;
                ...
                data[0] = grid->get(i, j, k);
                data[1] = grid->get(i, j, k + 1);
                data[2] = grid->get(i, j + 1, k + 1);
                data[3] = grid->get(i, j + 1, k);

                if (bndFlag & kDirectionXNegative) {
                ...
                i = dimx - 2;
                data[0] = grid->get(i + 1, j, k + 1);
                data[1] = grid->get(i + 1, j, k);
                data[2] = grid->get(i + 1, j + 1, k);
                data[3] = grid->get(i + 1, j + 1, k + 1);

                if (bndFlag & kDirectionXPositive) {
                    normal = Vector3f(1, 0, 0);
    ...
    if (bndFlag
        ...
        for (ssize_t k = 0; k < dimz-1; ++k) {
            for (ssize_t i = 0; i < dimx-1; ++i) {
                ...
                if (bndFlag & kDirectionYNegative) {
                ...
                j = dimy - 2;
                data[0] = grid->get(i + 1, j + 1, k);
                data[1] = grid->get(i, j + 1, k);
                data[2] = grid->get(i, j + 1, k + 1);
                data[3] = grid->get(i + 1, j + 1, k + 1);

                if (bndFlag & kDirectionYPositive) {
                    normal = Vector3f(0, 1, 0);
...
void marchingCubes( const NarrowbandLevelSetPtr& grid, REAL gridSize,
    const Vector3f& origin, TriangleMesh3* mesh, REAL isoValue, int bndFlag) {
